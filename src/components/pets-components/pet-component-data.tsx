...
export function PetComponentData({ editPetComponentState }: PetMainProps) {
  ...
  return (
    <div className="grid lg:grid-cols-2 md:grid-cols-1 min-[490px]:grid-cols-1 min-[320px]:grid-cols-1 grid-cols-2 lg:gap-x-8 gap-y-12 md:gap-y-12 sm:gap-y-12 ml-6 max-[382px]:ml-4 lg:ml-14 md:ml-14 sm:ml-6 mt-16 max-[382px]:mt-9">
      <>
                ...
                <div className="space-y-6 flex flex-col justify-center">
                  <div className="space-y-2">
                    ...
                    <p className="text-main lg:flex md:flex sm:flex hidden">
                      Left at {String(new Date(data["petLeftAt"]).getHours())}h
                      and {String(new Date(data["petLeftAt"]).getMinutes())}min
                    </p>
                    <p className="text-white lg:flex md:flex sm:flex hidden">
                      Leave at {String(new Date(data["petLeaveAt"]).getHours())}
                      h and {String(new Date(data["petLeaveAt"]).getMinutes())}
                      min
                    </p>
                  </div>
                  <button
